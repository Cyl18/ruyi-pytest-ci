FROM linuxdeepin/deepin:beige-25-riscv64-v1.5.0 AS builder
...
RUN apt-get update && apt-get upgrade -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" && apt-get install -y llvm-19-tools coreutils util-linux yq file expect sudo git make tar jq build-essential zstd && apt-get clean
